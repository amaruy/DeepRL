...
class ProgMccValueNetwork:
    ...
    def __init__(self, learning_rate, name='value_network'):
        ...
        self.hidden_1 = config.mcc_prog_value_hidden_size
        self.hidden_2 = config.mcc_prog_value_hidden_size
    ...
    def init_weights(self):
        ...
        """
        self.W1 = tf.compat.v1.get_variable("W1", [self.state_size, self.hidden_1], initializer=tf.initializers.GlorotUniform(seed=0))
        self.b1 = tf.compat.v1.get_variable("b1", [self.hidden_1], initializer=tf.zeros_initializer())
        self.W2 = tf.compat.v1.get_variable("W2", [self.hidden_1, self.hidden_2], initializer=tf.initializers.GlorotUniform(seed=0))
        self.b2 = tf.compat.v1.get_variable("b2", [self.hidden_2], initializer=tf.zeros_initializer())
        self.W3 = tf.compat.v1.get_variable("W3", [self.hidden_2, 1], initializer=tf.initializers.GlorotUniform(seed=0))
        self.b3 = tf.compat.v1.get_variable("b3", [1], initializer=tf.zeros_initializer())

    def restore_cartpole_value_weights(self):
        """
        Restores the weights for the Cartpole value network.
        """
